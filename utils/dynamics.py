...
class Dynamics(object):
    def __init__(self,
        ...
        self.temperature = tf.compat.v1.placeholder(TF_FLOAT, shape=())
    
        if not hmc:
            alpha = tf.get_variable(
            'alpha',
            initializer=tf.compat.v1.log(tf.constant(eps)),
    ...
    def _gen_mask(self, x):
        ...
        tf.convert_to_tensor(nb)
        return b.astype(TF_FLOAT), nb.astype(TF_FLOAT)
#
